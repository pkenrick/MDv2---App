class EditTaskController < UIViewController
  ...
  attr_accessor :task, :parent_controller, :table_view, :date_picker_visible, :title_field, :description_field, :date_picker, :current_date, :note_field, :monkey_button, :owner_monkey_view, :owner_monkey_text
  ...
  def initWithTask(task)
    ...
    cancel_edit_bar_button = UIBarButtonItem.alloc.initWithCustomView(cancel_button)
    self.navigationItem.leftBarButtonItem = cancel_edit_bar_button
  ...
  def tableView(tableView, cellForRowAtIndexPath: indexPath)
    ...
    when 0
      if indexPath.row == 0
        ...
        @description_field.text = task.details
    ...
    when 1
      if indexPath.row == 0
        ...
        if date_picker
          ...
          if task.due_date
            date_format = NSDateFormatter.alloc.init.setDateFormat("cccc, MMM d, hh:mm aa")
            cell.detailTextLabel.text = date_format.stringFromDate(task.due_date)
          else
